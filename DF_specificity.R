...
for (method in methods) {
  for (percentage in percentages){
    for (replicate in replicates){
      ...
      #read-in output DF on dataset containing synthetic doublets
      ...
      ##column 3 "sum"
      df[counter, "sum"] <- as.numeric(dim(output_DF)[1]) - sum(barcodes_synthetic_doublets[,1] %in% output_DF[,2])
      ...
      false_negatives <- as.numeric(dim(barcodes_synthetic_doublets)[1]) - sum(barcodes_synthetic_doublets[,1] %in% output_DF[,2])
      true_negatives <- total - as.numeric(dim(output_DF)[1]) - false_negatives
      false_positives <- as.numeric(dim(output_DF)[1]) - sum(barcodes_synthetic_doublets[,1] %in% output_DF[,2])
      
      df[counter, "specificity"] <- true_negatives/(true_negatives + false_positives)
      
      #save dataframe as csv file
      write.csv(df, "DF_specificity.csv")
