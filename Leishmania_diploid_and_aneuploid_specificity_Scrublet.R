...
plot <- ggplot(final_dataframe, aes(x= group, y= specificity, color = dataset)) +
  ...
  ggtitle("Specificity Leishmania (diploid) and Leishmania (aneuploid) dataset")

plot

ggsave(filename = "Leishmania_diploid_and_aneuploid_Specificity_Scrublet.png", 
       plot, 
       device = "png")
