...
#read-in csv file
df <- read.csv(file = "./original_VS_predicted_all.csv", sep = ",")

#only plot results of Plasmodium
final_dataframe <- as.data.frame(df[c(which(df$dataset == "Plasmodium")),])
...
plot <- ggplot(final_dataframe, aes(x= group, y= ratio, color = tool)) +
  ...
  ggtitle("Predicted VS original doublets - Plasmodium dataset")

plot

ggsave(filename = "Plasmodium_predicted_VS_original.png", 
       plot, 
       device = "png")
