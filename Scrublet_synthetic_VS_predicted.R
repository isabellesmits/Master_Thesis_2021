...
for (method in methods) {
  for (percentage in percentages){
    for (replicate in replicates){
      ...
      ##column 3 "sum"
      df[counter, "sum"] <- sum(barcodes_synthetic_doublets[,1] %in% output_Scrublet[,2])
      
      ##column 4 "ratio"
      amount_synthetic_doublets <- as.numeric(dim(barcodes_synthetic_doublets)[1])
      df[counter, "ratio"] <- sum(barcodes_synthetic_doublets[,1] %in% output_Scrublet[,2])/amount_synthetic_doublets
      
      #save dataframe as csv file
      write.csv(df, "Scrublet_synthetic_VS_predicted.csv")
    }
  }
}
