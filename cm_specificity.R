...
for (method in methods) {
  for (percentage in percentages){
    for (replicate in replicates){
      ...
      #read-in output counts method on dataset containing synthetic doublets
      ...
      df[counter, "sample"] <- as.character(paste0(method,
                                                   ...
                                                   replicate))
      ##column 3 "sum"
      df[counter, "sum"] <- as.numeric(dim(output_cm)[1]) - sum(barcodes_synthetic_doublets[,1] %in% output_cm[,2])
      ...
      false_negatives <- as.numeric(dim(barcodes_synthetic_doublets)[1]) - sum(barcodes_synthetic_doublets[,1] %in% output_cm[,2])
      true_negatives <- total - as.numeric(dim(output_cm)[1]) - false_negatives
      false_positives <- as.numeric(dim(output_cm)[1]) - sum(barcodes_synthetic_doublets[,1] %in% output_cm[,2])
      
      df[counter, "specificity"] <- true_negatives/(true_negatives + false_positives)
      
      #save dataframe as csv file
      write.csv(df, "cm_specificity.csv")
