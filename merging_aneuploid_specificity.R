...
datasets=c( "Leishmania_aneuploid")
tools=c("DD", "DF", "Scrublet", "Solo", "cm")
parameters=c("aneuploid_specificity")


for (parameter in parameters) {
  counter = 0
  names_dataframe <- paste0("df_", c(1:5))
  for (tool in tools) {
  ...
  binding_dataframe <- do.call("rbind", list(df_1, df_2, df_3, df_4, df_5))
  assign(paste0(parameter, "_all"), data.frame(binding_dataframe))
}

write.csv(aneuploid_specificity_all, "./aneuploid_specificity_all.csv")
